...
-- Date: 2026-04-18
-- Name: full_scd2.usp_MergeUsers
-- Description: Full SCD2 merge from full_scd2.stage_Users to full_scd2.prod_Users.
-- Version: 1.0
-- -----------------------------------------------------------------------------

CREATE PROCEDURE [full_scd2].[usp_MergeUsers]
...
BEGIN
	...
	DECLARE @ProcObjectId INT = OBJECT_ID(N'[full_scd2].[usp_MergeUsers]');
	...
	BEGIN
		THROW 55101, 'Required table [full_scd2].[stage_Users] does not exist.', 1;
	END;

	IF OBJECT_ID(N'[full_scd2].[prod_Users]', N'U') IS NULL
	BEGIN
		THROW 55102, 'Required table [full_scd2].[prod_Users] does not exist.', 1;
	END;
	...
	BEGIN
		EXEC [monitor].[usp_ProcedureRunStart]
			...
			@ProcedureName = N'full_scd2.usp_MergeUsers',
	...
	BEGIN TRY
		...
		FROM [full_scd2].[prod_Users] AS p
		WHERE p.[IsActive] = 1
			AND NOT EXISTS
			(
				SELECT 1
		...
		INSERT INTO [full_scd2].[prod_Users]
		(
		...
		FROM [full_scd2].[stage_Users] AS s
		LEFT JOIN [full_scd2].[prod_Users] AS p
