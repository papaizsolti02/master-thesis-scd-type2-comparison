...
-- Date: 2026-04-18
-- Name: full_scd2.usp_ProcessUsers
-- Description: Full SCD2 staging procedure.
-- Version: 1.0
-- -----------------------------------------------------------------------------

CREATE PROCEDURE [full_scd2].[usp_ProcessUsers]
...
BEGIN
    ...
    DECLARE @ProcObjectId INT = OBJECT_ID(N'[full_scd2].[usp_ProcessUsers]');
    ...
    IF OBJECT_ID(N'[full_scd2].[stage_Users]', N'U') IS NULL
    BEGIN
        THROW 53001, 'Required table [full_scd2].[stage_Users] does not exist.', 1;
    END;

    IF OBJECT_ID(N'[full_scd2].[raw_Users]', N'U') IS NULL
    BEGIN
        THROW 53002, 'Required table [full_scd2].[raw_Users] does not exist.', 1;
    END;

    IF OBJECT_ID(N'[config].[Countries]', N'U') IS NULL
    BEGIN
        THROW 53003, 'Required table [config].[Countries] does not exist.', 1;
    END;

    IF OBJECT_ID(N'[config].[SubscriptionTiers]', N'U') IS NULL
    BEGIN
        THROW 53004, 'Required table [config].[SubscriptionTiers] does not exist.', 1;
    END;

    IF OBJECT_ID(N'[config].[PaymentMethods]', N'U') IS NULL
    BEGIN
        THROW 53005, 'Required table [config].[PaymentMethods] does not exist.', 1;
    END;
    ...
    BEGIN
        EXEC [monitor].[usp_ProcedureRunStart]
            ...
            @ProcedureName = N'full_scd2.usp_ProcessUsers',
    ...
    BEGIN TRY
        ...
        TRUNCATE TABLE [full_scd2].[stage_Users];

        INSERT INTO [full_scd2].[stage_Users]
        (
            [FirstName],
            [LastName],
            [Email],
        ...
        FROM [full_scd2].[stage_Users] AS s;
