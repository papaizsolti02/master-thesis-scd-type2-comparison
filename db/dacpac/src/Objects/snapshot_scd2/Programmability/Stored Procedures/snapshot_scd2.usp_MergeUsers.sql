...
BEGIN
    ...
    BEGIN TRY
        ...
        FROM [snapshot_scd2].[stage_Users] s
        INNER JOIN [snapshot_scd2].[prod_Users] p
            ON p.IsActive = 1
        ...
        FROM [snapshot_scd2].[prod_Users] p
        INNER JOIN [snapshot_scd2].[stage_Users] s
        ...
        SET
            ...
            p.LastRefreshedDate = @AsOfDate
        FROM [snapshot_scd2].[prod_Users] p
        LEFT JOIN [snapshot_scd2].[stage_Users] s
            ON ISNULL(p.Email, '') = ISNULL(s.Email, '')
            AND ISNULL(p.Username, '') = ISNULL(s.Username, '')
        WHERE p.IsActive = 1
          AND s.Email IS NULL;
        ...
        SELECT
            ...
            s.Rowhash
        FROM [snapshot_scd2].[stage_Users] s
        LEFT JOIN [snapshot_scd2].[prod_Users] p
            ON p.IsActive = 1
            AND ISNULL(p.Email, '') = ISNULL(s.Email, '')
            AND ISNULL(p.Username, '') = ISNULL(s.Username, '')
        WHERE p.Id IS NULL
           OR ISNULL(p.Rowhash, 0x0) <> ISNULL(s.Rowhash, 0x0);
